...
const str =
  "hello, i am <@=user@>, from <@=location@>, like <@=like.a@> and <@=like.b@>.";
...
function template(tpl, state = {}) {
  ...
  // }
  // fun(state);
  return tpl.replace(/<@=[\w\.]+@>/g,text=>{
    return text.replace(/[^\w\.]+/g,'').split('.').reduce((a,b)=>{
      return a[b]
    },state)
  })
}

const result = template(str, {
  user: "jojo",
  location: "china",
  like: { a: "apple", b: "pencil" },
});

console.log("====== 模板引擎 ======",result);
console.log(
  "result --> " , result ===
    "hello, i am jojo, from china, like apple and pencil."
);
console.log(
  "want ----> " , want ===
    "hello, i am jojo, from china, like apple and pencil."
